...
pub fn main() -> Result<(), anyhow::Error> {
    ...
    let mut config = init(&CONFIG_PATH)
    ...
    loop {
        ...
        find_offending_processes(&config)
            .context("error while examining offending processes")?;
    }
}

pub struct UserInstructions {
    user_name: String,
    instructions: HashMap<Binary, Box<[config::Interval]>>,
}
...
pub fn init<P: AsRef<Path>>(path: &P) -> Result<PerUid, anyhow::Error> {
    ...
    for watch in config.watch {
        ...
        let per_binary = per_uid.entry(uid).or_insert_with(|| UserInstructions::new(watch.user));
        per_binary.instructions.insert(watch.binary, watch.permitted.into_boxed_slice());
    }
    Ok(per_uid)
}
...
pub fn find_offending_processes(config: &PerUid) -> Result<(), anyhow::Error> {
    ...
    for proc in processes {
        ...
        let Some(per_binary) = config.get(&uid)
        else {
        ...
        let Some(intervals) = per_binary.instructions.get(&exe)
        else {
            // Nothing to watch for this binary.
            continue;
        };
        log::info!("found binary {} for user {}", exe, per_binary.user_name);
        if let Some(duration) = intervals.iter().filter_map(|interval| interval.remaining(now)).next() {
            ...
            if duration < std::time::Duration::from_secs(300) {
                ...
                if let Err(err) = notify(&per_binary.user_name, &format!("{} will quit in {} minutes", exe, minutes), notify::Urgency::Significant) {
            ...
            if let Err(err) = notify(&per_binary.user_name, &format!("{} is not permitted at this time, stopping it", exe), notify::Urgency::Significant) {
                log::warn!(target: "notify", "failed to notify user {}: {:?}", per_binary.user_name, err)
            }
            if let Err(err) = kill_tree::blocking::kill_tree_with_config(proc.pid as u32, &kill_tree::Config { signal: "SIGKILL".to_string(), ..Default::default() }) {
                log::warn!(target: "notify", "failed to kill process {:?}: {:?}", exe.0, err)
            }
